-- ============================================================================
-- PAYROLL VERSIONING COMPOSITE TYPES
-- Create composite types so Hasura can track the versioning functions
-- ============================================================================

-- Composite type for create_payroll_version function result
CREATE TYPE public.payroll_version_result AS (
    new_payroll_id uuid,
...
-- Composite type for get_latest_payroll_version function result
CREATE TYPE public.latest_payroll_version_result AS (
    id uuid,
...
-- Composite type for get_payroll_version_history function result
CREATE TYPE public.payroll_version_history_result AS (
    id uuid,
    name character varying(255),
    version_number integer,
    go_live_date date,
...
-- Composite type for activate_payroll_versions function result
CREATE TYPE public.payroll_activation_result AS (
    payroll_id uuid,
    action_taken text,
    version_number integer
);
...
-- ============================================================================
-- UPDATED FUNCTIONS USING COMPOSITE TYPES
-- ============================================================================

-- Function to create a new payroll version (using composite type)
...
) RETURNS SETOF public.payroll_version_result
LANGUAGE plpgsql
AS $$
DECLARE
    ...
    v_result public.payroll_version_result;
BEGIN
    ...
    -- Return the result
    v_result.new_payroll_id := v_new_payroll_id;
    v_result.new_version_number := v_new_version_number;
    v_result.old_payroll_id := p_original_payroll_id;
    v_result.dates_deleted := v_dates_deleted;
    v_result.message := format('Created version %s of payroll with go-live date %s', v_new_version_number, p_go_live_date);
    
    RETURN NEXT v_result;
END;
$$;

-- Function to get the latest active version of a payroll (using composite type)
CREATE OR REPLACE FUNCTION public.get_latest_payroll_version(p_payroll_id uuid)
RETURNS SETOF public.latest_payroll_version_result
LANGUAGE plpgsql
AS $$
BEGIN
    RETURN QUERY
...
-- Function to get payroll version history (using composite type)
CREATE OR REPLACE FUNCTION public.get_payroll_version_history(p_payroll_id uuid)
RETURNS SETOF public.payroll_version_history_result
LANGUAGE plpgsql
AS $$
BEGIN
    RETURN QUERY
...
-- Function to activate payroll versions (cron job) - using composite type
CREATE OR REPLACE FUNCTION public.activate_payroll_versions()
RETURNS SETOF public.payroll_activation_result
LANGUAGE plpgsql
AS $$
DECLARE
    v_payroll RECORD;
    v_result public.payroll_activation_result;
BEGIN
    ...
    LOOP
        ...
        v_result.payroll_id := v_payroll.id;
        v_result.action_taken := 'deactivated_superseded_version';
        v_result.version_number := v_payroll.version_number;
        RETURN NEXT v_result;
    END LOOP;
    ...
    LOOP
        v_result.payroll_id := v_payroll.id;
        v_result.action_taken := 'activated_new_version';
        v_result.version_number := v_payroll.version_number;
        RETURN NEXT v_result;
    END LOOP;
END;
