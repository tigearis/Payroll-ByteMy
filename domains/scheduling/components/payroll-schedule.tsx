...
// Import types and GraphQL queries from main app
import {
  PayrollEvent,
  CalendarView,
  WeekOrientation,
...
// Types from main app payroll data
...
export default function PayrollSchedule() {
  ...
  // GraphQL Data Fetching using main app queries
  const {
    data: payrollsData,
  ...
  const events = useMemo(() => {
    ...
    payrolls.forEach((payroll) => {
      payroll.payroll_dates.forEach((dateInfo) => {
        // Processing event
        eventList.push({
          ...
          type: "processing",
          consultant: {
            id: payroll.primaryConsultantUser?.id || "",
            name: payroll.primaryConsultantUser?.name || "Unassigned",
          },
  ...
  const clients = useMemo(() => {
    ...
    payrolls.forEach((payroll) => {
      if (!clientMap.has(payroll.client.id)) {
  ...
  const consultants = useMemo(() => {
    ...
    payrolls.forEach((payroll) => {
      const consultant = payroll.primaryConsultantUser;
