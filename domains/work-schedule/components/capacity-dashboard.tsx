...
import { CapacityDashboardSkeleton } from "./loading-states";
...
interface CapacityDashboardProps {
  ...
  onUpdateWorkSchedule?: (
    scheduleId: string,
    updates: Partial<WorkScheduleWithCapacity> & { userId?: string }
  ) => Promise<void>;
...
const calculateTeamMetrics = (
  ...
  const totalTeamCapacity = capacities.reduce(
    (sum, c) => sum + c.totalPayrollCapacity,
    0
  );
  const totalAssignedHours = capacities.reduce(
    (sum, c) => sum + c.currentlyAssignedHours,
    0
  );
  const teamUtilization =
    totalTeamCapacity > 0 ? (totalAssignedHours / totalTeamCapacity) * 100 : 0;
  const availableCapacity = totalTeamCapacity - totalAssignedHours;
  const overallocatedMembers = capacities.filter(
    c => c.utilizationPercentage > 100
  ).length;
  const underutilizedMembers = capacities.filter(
    c => c.utilizationPercentage < 60
  ).length;
...
export function ConsultantCapacityCard({
  ...
  const utilizationColor = getCapacityStatusColor(
  ...
  return (
    <Card
      className={`transition-all duration-200 ${hasConflicts ? "ring-2 ring-orange-200" : ""}`}
    >
          ...
          <div className="text-right">
            <div
              className={`text-lg font-bold ${
                utilizationColor.includes("red")
                  ? "text-red-600"
                  : utilizationColor.includes("yellow")
                    ? "text-yellow-600"
                    : utilizationColor.includes("green")
                      ? "text-green-600"
                      : "text-blue-600"
              }`}
            >
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <div className="flex justify-between text-sm">
            <span>Available</span>
            <span
              className={`font-medium ${capacity.availableCapacityHours > 0 ? "text-green-600" : "text-red-600"}`}
            >
        ...
        <div className="flex space-x-2">
          ...
          {(capacity.utilizationPercentage > 90 ||
...
export function TeamMetricsOverview({
  ...
  const utilizationStatus =
    metrics.teamUtilization <= 70
      ? "Under-utilized"
      : metrics.teamUtilization <= 85
        ? "Well-balanced"
        : metrics.teamUtilization <= 100
          ? "High utilization"
          : "Overloaded";
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            <div>
              <p className="text-2xl font-bold">
                {metrics.totalTeamCapacity.toFixed(0)}h
              </p>
      ...
      <Card>
        <CardContent className="p-4">
          <div className="flex items-center space-x-2">
            <TrendingUp
              className={`w-5 h-5 ${statusColor.replace("text-", "")}`}
            />
            <div>
      ...
      <Card className="md:col-span-2">
        <CardContent className="p-4">
          <div className="space-y-2">
            ...
            <Progress
              value={Math.min(metrics.teamUtilization, 100)}
              className="h-2"
            />
            <div className="flex justify-between text-xs text-muted-foreground">
...
function MemberDetailsView({
  ...
  const [tempAdminPercentage, setTempAdminPercentage] = useState(
    member?.defaultAdminTimePercentage || 12.5
  );
  const [scheduleUpdates, setScheduleUpdates] = useState<
    Record<string, Partial<WorkScheduleWithCapacity>>
  >({});
  ...
  const handleScheduleChange = (
    scheduleId: string,
    field: keyof WorkScheduleWithCapacity,
    value: any
  ) => {
    setScheduleUpdates(prev => ({
      ...prev,
          ...
          <div className="flex items-center justify-between">
            ...
            {!editingAdminTime ? (
              <Button
                variant="outline"
                size="sm"
                onClick={() => setEditingAdminTime(true)}
              >
              ...
              <div className="flex items-center space-x-2">
                ...
                <Button
                  size="sm"
                  onClick={handleSaveAdminTime}
                  disabled={savingAdminTime}
                >
                  {savingAdminTime ? (
        ...
        <CardContent>
          <div className="space-y-4">
            <div>
              ...
              {!editingAdminTime ? (
                <p className="text-2xl font-semibold mt-1">
                  {member.defaultAdminTimePercentage}%
                </p>
              ) : (
                <div className="space-y-2 mt-2">
                  <div className="flex items-center space-x-4">
                    <Input
                      ...
                      onChange={e =>
                        setTempAdminPercentage(parseFloat(e.target.value) || 0)
                      }
                      className="flex-1"
                    />
                  ...
                  <p className="text-sm text-muted-foreground">
                    Recommended: {getPositionAdminPercentage(member.position)}%
                    for {getPositionDisplayName(member.position)}
                  </p>
                </div>
              )}
            </div>
            <Alert>
              ...
              <AlertDescription>
                Admin time is automatically calculated for each work day unless
                manually overridden in the schedule below.
              </AlertDescription>
            </Alert>
          </div>
        </CardContent>
        ...
        <CardContent>
          <div className="space-y-4">
            {daysOfWeek.map((day, index) => {
              ...
              const isEditing =
                editingSchedule === (schedule?.id || `new-${index}`);
              const updates =
                scheduleUpdates[schedule?.id || `new-${index}`] || {};
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {!isEditing ? (
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          setEditingSchedule(schedule?.id || `new-${index}`)
                        }
                      >
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <Button
                          size="sm"
                          onClick={() =>
                            handleSaveSchedule(schedule?.id || `new-${index}`)
                          }
                          disabled={
                    ...
                    <div className="grid grid-cols-3 gap-4 text-sm">
                      <div>
                        ...
                        <p className="font-medium">
                          {(schedule?.workHours || 0).toFixed(2)}h
                        </p>
                      </div>
                      <div>
                        <p className="text-muted-foreground">Admin Time</p>
                        <p className="font-medium">
                          {(schedule?.adminTimeHours || 0).toFixed(2)}h
                      ...
                      <div>
                        ...
                        <p className="font-medium">
                          {(schedule?.payrollCapacityHours || 0).toFixed(2)}h
                        </p>
                      </div>
                    </div>
                  ) : (
                    <div className="space-y-3">
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <Input
                            ...
                            step="0.5"
                            value={
                              updates.workHours ?? schedule?.workHours ?? 0
                            }
                            onChange={e =>
                        ...
                        <div>
                          ...
                          <Input
                            ...
                            step="0.5"
                            value={
                              updates.adminTimeHours ??
                              schedule?.adminTimeHours ??
                              0
                            }
                            onChange={e =>
                              handleScheduleChange(
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <Label className="text-sm">
                          Use default admin time percentage
                        </Label>
                      </div>
                    </div>
                  )}
      ...
      <Card>
        ...
        <CardContent>
          {member.assignedPayrolls.length === 0 ? (
            <p className="text-muted-foreground">
              No payrolls currently assigned
            </p>
          ) : (
            <div className="space-y-2">
...
export function CapacityDashboard({
  ...
  const [filterPosition, setFilterPosition] = useState<UserPosition | "all">(
    "all"
  );
  ...
  const teamCapacities = useMemo(() => {
    ...
    return teamMembers.map(member => {
      const totalWorkHours = member.workSchedules.reduce(
        (sum, s) => sum + s.workHours,
        0
      );
      const totalAdminHours = member.workSchedules.reduce(
        (sum, s) => sum + s.adminTimeHours,
        0
      );
      const totalPayrollCapacity = member.workSchedules.reduce(
        (sum, s) => sum + s.payrollCapacityHours,
        0
      );
      const currentlyAssignedHours = member.assignedPayrolls.reduce(
        (sum, p) => sum + p.processingTime,
        0
      );
      const availableCapacityHours = Math.max(
        0,
        totalPayrollCapacity - currentlyAssignedHours
      );
      const utilizationPercentage =
        totalPayrollCapacity > 0
          ? (currentlyAssignedHours / totalPayrollCapacity) * 100
          : 0;
      const adminTimePercentage =
        totalWorkHours > 0 ? (totalAdminHours / totalWorkHours) * 100 : 0;
      const processingWindowDays = member.workSchedules.filter(
        s => s.workHours > 0
      ).length;
  ...
  const filteredCapacities = useMemo(() => {
    return teamCapacities.filter(item => {
      if (filterPosition !== "all" && item.member.position !== filterPosition)
        return false;
          ...
          <div className="text-center space-y-4">
            ...
            <div>
              ...
              <p className="text-gray-600 mt-1">
                No consultants are available for the selected date range. Try
                adjusting your filters or date range.
              </p>
            </div>
            {onRefresh && (
              <Button onClick={onRefresh} variant="outline" className="mt-4">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <div className="flex items-center space-x-4">
            <div className="flex items-center space-x-2">
              ...
              <select
                ...
                onChange={e =>
                  setFilterPosition(e.target.value as UserPosition | "all")
                }
                  ...
                  <tbody>
                    {filteredCapacities.map(({ member, capacity }) => (
                      <tr key={member.id} className="border-b hover:bg-gray-50">
                        ...
                        <td className="p-2">
                          <Badge
                            className={`${getPositionColor(member.position)} text-xs`}
                          >
                            {getPositionDisplayName(member.position)}
                          </Badge>
                        </td>
                        ...
                        <td className="text-right p-2">
                          <span
                            className={
                              capacity.availableCapacityHours > 0
                                ? "text-green-600"
                                : "text-red-600"
                            }
                          >
        ...
        <TabsContent value="analytics" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Card>
              ...
              <CardContent>
                <div className="space-y-3">
                  {["0-50%", "51-75%", "76-90%", "91-100%", "100%+"].map(
                    range => {
                      ...
                      const percentage =
                        teamCapacities.length > 0
                          ? (count / teamCapacities.length) * 100
                          : 0;
              ...
              <CardContent>
                <div className="space-y-3">
                  {[
                    ...
                    const members = teamCapacities.filter(
                      item => item.member.position === pos
                    );
                    const avgUtilization =
                      members.length > 0
                        ? members.reduce(
                            (sum, item) =>
                              sum + item.capacity.utilizationPercentage,
                            0
                          ) / members.length
                        : 0;
                    ...
                    return (
                      ...
                      >
                        <Badge
                          className={`${getPositionColor(pos as UserPosition)} text-xs`}
                        >
        ...
        {selectedMember && (
          <TabsContent value="details" className="space-y-4">
            {(() => {
              const memberData = teamCapacities.find(
                tc => tc.member.id === selectedMember
              );
                ...
                <MemberDetailsView
                  ...
                  onUpdateWorkSchedule={async (scheduleId, updates) => {
                    if (onUpdateWorkSchedule && selectedMember) {
                      await onUpdateWorkSchedule(scheduleId, {
                        ...updates,
