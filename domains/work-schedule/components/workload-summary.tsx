...
export const WorkloadSummary: React.FC<WorkloadSummaryProps> = ({
  ...
  const stats = useMemo(() => {
    const totalAssignments = periodData.reduce(
      (sum, day) => sum + day.assignments.length,
      0
    );
    const totalAssignedHours = periodData.reduce(
      (sum, day) =>
        sum +
        day.assignments.reduce(
          (daySum, assignment) => daySum + assignment.processingTime,
          0
        ),
      0
    );
    const totalCapacity = periodData.reduce(
    ...
    const overallocatedDays = periodData.filter(day => {
      const dayAssignedHours = day.assignments.reduce(
        (sum, assignment) => sum + assignment.processingTime,
        0
      );
      return (
        day.payrollCapacityHours > 0 &&
        dayAssignedHours / day.payrollCapacityHours > 1
      );
    }).length;

    const avgDailyHours =
      periodData.length > 0 ? totalAssignedHours / periodData.length : 0;

    // Group by client
    const clientSummary = periodData.reduce(
      (acc, day) => {
    ...
    // Group by status
    const statusSummary = periodData.reduce(
      (acc, day) => {
        day.assignments.forEach(assignment => {
  ...
  const getPeriodLabel = () => {
    switch (viewPeriod) {
      case "day":
        return safeFormatDate(currentDate, "dd MMM yyyy");
      case "week":
        const weekStart = startOfWeek(currentDate, { weekStartsOn: 1 });
        return `Week of ${safeFormatDate(weekStart, "dd MMM yyyy")}`;
      case "month":
    ...
    <Card className={cn("w-full", className)}>
      ...
      <CardContent className="space-y-6">
        ...
        {viewPeriod === "day" && stats.allAssignments.length > 0 && (
          <div className="space-y-3">
            <h4 className="font-medium">
              Today's Assignments ({stats.allAssignments.length})
            </h4>
        ...
        {viewPeriod === "week" && stats.allAssignments.length > 0 && (
          <div className="space-y-3">
            <h4 className="font-medium">
              Week's Assignments ({stats.allAssignments.length})
            </h4>
