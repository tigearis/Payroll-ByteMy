...
export const POST = withEnhancedAuth(
  async (req: NextRequest, context) => {
    try {
      ...
      const { userId } = context; // Already authenticated and role-verified
      ...
      if (userData.pending_leaves?.length > 0) {
        blockingDependencies.push(
          `${userData.pending_leaves.length} pending leave approvals`
        );
      }
  ...
  {
    permission: "custom:admin:manage", // Org admins and above can delete staff
  }
);
