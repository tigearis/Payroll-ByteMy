...
export function PayrollListCard({
  ...
  const searchFilteredPayrolls = safePayrolls.filter((payroll) => {
    ...
    return (
      ...
      (payroll.primaryConsultantUser?.name &&
        payroll.primaryConsultantUser.name
          .toLowerCase()
      ...
      searchFilteredPayrolls
        .filter((p) => {
          ...
          const isConsultantMatch =
            ...
            (p.primaryConsultantUser?.id &&
              p.primaryConsultantUser.id.toString() === selectedConsultant);
    ...
    new Map(
      searchFilteredPayrolls
        .filter((p) => {
          ...
          return isClientMatch && isPayrollMatch && p.primaryConsultantUser;
        })
        .map((p) => [
          p.primaryConsultantUser?.id,
          {
            id: p.primaryConsultantUser?.id,
            name: p.primaryConsultantUser?.name,
          },
        ])
    ).values()
  );
  ...
  const filteredPayrolls = safePayrolls.filter((payroll) => {
    ...
    const isConsultantMatch =
      selectedConsultant === "all" ||
      (payroll.primaryConsultantUser?.id &&
        payroll.primaryConsultantUser.id.toString() === selectedConsultant);
              ...
              onValueChange={(value) => {
                ...
                if (value !== "all" && selectedPayroll !== "all") {
                  const consultantWorksWithPayroll = payrolls.some(
                    (p) =>
                      p.id.toString() === selectedPayroll &&
                      p.primaryConsultantUser?.id.toString() === value
                  );
            ...
            {filteredPayrolls.length > 0 ? (
              filteredPayrolls.map((payroll) => (
                <TableRow key={payroll.id}>
                  ...
                  <TableCell>
                    {payroll.primaryConsultantUser?.name || "N/A"}
                  </TableCell>
                  <TableCell>
