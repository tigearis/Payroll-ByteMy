...
CREATE FUNCTION public.get_latest_payroll_version(p_payroll_id uuid) RETURNS TABLE(id uuid, name character varying, version_number integer, go_live_date date, active boolean)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY
...
CREATE FUNCTION public.get_payroll_version_history(p_payroll_id uuid) RETURNS TABLE(id uuid, name character varying, version_number integer, go_live_date date, superseded_date date, version_reason text, active boolean, is_current boolean)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY
