...
export function useEnhancedPermissions() {
  ...
  const checkPermission = useCallback((permission: CustomPermission): boolean => {
  ...
  const checkMultiplePermissions = useCallback((checks: Array<{ key: string, permission: CustomPermission }>) => {
    return checks.reduce((acc, { key, permission }) => {
  ...
  const hasAnyPermissions = useCallback((permissions: CustomPermission[]): boolean => {
    return permissions.some(permission => checkPermission(permission));
  }, [checkPermission]);

  // Check if user has all of the provided permissions
  const hasAllPermissions = useCallback((permissions: CustomPermission[]): boolean => {
...
export function EnhancedPermissionProvider({ children }: { children: React.ReactNode }) {
...
export function useResourceAccess(resourceType: 'payroll' | 'client', resourceId?: string) {
  ...
  const canRead = useMemo(() => {
    ...
    if (resourceId && userMetadata?.customAccess) {
      if (resourceType === 'payroll' && userMetadata.customAccess.restrictedPayrolls) {
        return !userMetadata.customAccess.restrictedPayrolls.includes(resourceId);
      }
      if (resourceType === 'client' && userMetadata.customAccess.allowedClients) {
  ...
  return {
    canRead,
    canWrite,
    canDelete: isLoaded ? checkPermission(`custom:${resourceType}:delete` as CustomPermission) && canRead : false,
    isLoading: !isLoaded
  };
}