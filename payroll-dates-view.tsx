...
export function PayrollDatesView({
  ...
  const { data: familyRootData } = useQuery(GET_FAMILY_ROOT_ID, {
    variables: { payrollId },
    skip: !showFamilyDates,
  });

  const familyRootId =
    familyRootData?.payrolls?.[0]?.parent_payroll_id || payrollId;
  ...
  } = useQuery(GET_PAYROLL_DATES_SPLIT, {
    variables: {
      familyRootId,
      today,
    },
  ...
  } = useQuery(GET_PAYROLL_FAMILY_DATES, {
    variables: { payrollId },
    skip: !showFamilyDates,
  });
  ...
  if (showFamilyDates && splitData) {
    // Use the pre-split data from the efficient query
    pastDates = splitData.past || [];
    futureDates = splitData.future || [];
    allDates = [...pastDates, ...futureDates];
  } else if (showFamilyDates && familyData) {
    // Fallback to client-side filtering
    allDates = familyData.payroll_dates || [];
    ...
    allDates = singleData.payroll_dates || [];
    const currentDate = startOfDay(new Date());
    pastDates = allDates.filter((date) => {
      const adjustedDate = startOfDay(parseISO(date.adjusted_eft_date));
      return adjustedDate < currentDate;
