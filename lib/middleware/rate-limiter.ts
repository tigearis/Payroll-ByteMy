...
export class RateLimiter {
  ...
  private readonly ROUTE_LIMITS: { [pattern: string]: RateLimitConfig } = {
    ...
    "/api/update-user-role": {
      requests: 3,
      window: 60000,
      message: "Role changes are strictly limited",
    },
    ...
    "/api/staff/delete": {
      requests: 3,
      window: 300000,
      message: "Staff deletion is strictly limited",
    },
    ...
    "/api/admin/api-keys": {
      requests: 5,
      window: 300000,
      message: "API key operations are strictly limited",
    },
    ...
    "/api/developer/reset-to-original": {
      requests: 3,
      window: 600000,
      message: "Reset operations are extremely limited",
    },
  ...
  async applyRateLimit(
    ...
    if (!result.success) {
      const config =
        customConfig ||
        this.getRouteConfig(this.normalizeRoute(request.nextUrl.pathname));
