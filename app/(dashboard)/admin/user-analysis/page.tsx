...
export default function UserAnalysisPage() {
  ...
  const analyzeUser = async () => {
    ...
    try {
      const response = await fetch(
        `/api/admin/user-management?email=${encodeURIComponent(email)}&action=analyze`
      );
      ...
      const candidatesResponse = await fetch(
        `/api/admin/user-management?email=${encodeURIComponent(email)}&action=get_reassignment_candidates`
      );
  ...
  return (
    <div className="container mx-auto py-8 space-y-6">
      <PageHeader
        title="User Analysis & Management"
        description="Analyze user dependencies and safely reassign before deletion"
        breadcrumbs={[
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <strong>ID:</strong> {analysis.user.id}
                  </div>
                  <div>
                    <strong>Email:</strong> {analysis.user.email}
                  </div>
                  <div>
                    <strong>Name:</strong> {analysis.user.firstName}{" "}
                    {analysis.user.lastName}
                  </div>
                  <div>
                    <strong>Role:</strong> <Badge>{analysis.user.role}</Badge>
                  </div>
                  <div>
                  ...
                  <div>
                    <strong>Staff:</strong>{" "}
                    {analysis.user.isStaff ? "✅" : "❌"}
                  </div>
                  <div>
                    <strong>Clerk ID:</strong>{" "}
                    {analysis.user.clerkUserId || "None"}
                  </div>
                  <div>
                    <strong>Created:</strong>{" "}
                    {new Date(analysis.user.createdAt).toLocaleDateString()}
                  </div>
                </div>
              </CardContent>
            </Card>
                    ...
                    <div>
                      ...
                      <ul className="mt-2 space-y-1">
                        {analysis.blockers.map((blocker, index) => (
                          <li
                            key={index}
                            className="text-sm text-orange-600 ml-4"
                          >
                            • {blocker}
                          </li>
                        ))}
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  >
                    {deleting
                      ? "Deleting User..."
                      : "Reassign Data & Delete User"}
                  </Button>

                  <Alert>
                    <AlertDescription>
                      ⚠️ This action is irreversible. The user will be deleted
                      from both the database and Clerk, and all their data will
                      be reassigned to the selected user.
                    </AlertDescription>
                  </Alert>
                </CardContent>
              </Card>
