...
export const POST = withAuth(async (req, session) => {
  try {
    ...
    switch (action) {
      ...
      case 'bulk_update':
        return await handleBulkUpdate(userIds, workDay, workHours, adminTimeHours, payrollCapacityHours);
      ...
      default:
        return NextResponse.json(
          { error: 'Invalid action. Use: bulk_create, bulk_update, or bulk_delete' },
          { status: 400 }
        );
    }
...
async function handleBulkCreate(schedules: any[], defaultUserId: string) {
  ...
  const validatedSchedules = schedules.map(schedule => {
    const { userId = defaultUserId, workDay, workHours, adminTimeHours, payrollCapacityHours, usesDefaultAdminTime } = schedule;
...
async function handleBulkUpdate(userIds: string[], workDay: string, workHours: number, adminTimeHours?: number, payrollCapacityHours?: number) {
  ...
  const data = await executeTypedMutation<BulkUpdateWorkScheduleCapacityMutation>(
    BulkUpdateWorkScheduleCapacityDocument,
    {
      userIds,
      workDay,
