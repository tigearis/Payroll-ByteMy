...
interface StaffListRequest {
  ...
  filters?: StaffFilters | undefined;
  orderBy?: "computedName" | "email" | "role" | "createdAt" | "updatedAt" | undefined;
  orderDirection?: "ASC" | "DESC" | undefined;
  includeStats?: boolean | undefined;
  includeNonStaff?: boolean | undefined;
}
...
async function handleStaffListRequest(
  ...
  try {
    ...
    const userRole =
      session.role ||
      session.defaultRole ||
      session.hasuraClaims?.["x-hasura-default-role"] ||
      session.hasuraClaims?.["x-hasura-role"];
      ...
      return NextResponse.json(
        {
          ...
          debug: { userRole, sessionKeys: Object.keys(session) },
        },
        { status: 403 }
      );
    }
    ...
    if (params.includeNonStaff && !isDeveloper) {
      return NextResponse.json(
        {
          success: false,
          error: "Developer access required for all users view",
        },
        { status: 403 }
      );
    }
    ...
    const limit = Math.min(
      Math.max(parseInt(String(params.limit || 20)), 1),
      100
    ); // Cap at 100
    ...
    const orderBy = params.orderBy || "computedName";
    ...
    if (search) {
      ...
      totalCount =
        (staffData as SearchUsersPaginatedQuery).searchAggregate?.aggregate
          ?.count || 0;
      ...
      totalCount =
        (staffData as GetStaffPaginatedQuery).staffAggregate?.aggregate
          ?.count || 0;
    }
...
export const GET = withAuth(async (req: NextRequest, session) => {
  ...
  const orderBy = (searchParams.get("orderBy") as any) || "computedName";
...
export const POST = withAuth(async (req: NextRequest, session) => {
  try {
    ...
    const params: StaffListRequest = {
      ...
      orderBy: body.orderBy || "computedName",
...
function formatStatsData(statsData: GetUserStatusDashboardStatsQuery) {
  return {
    ...
    recentChanges:
      statsData.recentStatusChanges?.map(user => ({
        id: user.id,
    ...
    roleDistribution:
      statsData.byRole?.reduce((acc: any, user: any) => {
