    ...
    const mutation = `
      mutation UpdateUserPosition(
        ...
        updateUsersByPk(
          ...
          _set: {
            # position: $position
            # defaultAdminTimePercentage: $adminPercentage
            updatedAt: "now()"
          }
        ) {
          id
...
export const POST = withAuth(async (req, session) => {
  try {
    ...
    const updates = userIds.map((userId: string) => ({
      ...
      adminPercentage: adminPercentage || POSITION_ADMIN_DEFAULTS[position as UserPosition] || 12.5
    }));
...
function getPositionDescription(position: UserPosition): string {
  const descriptions: Record<UserPosition, string> = {
    'consultant': 'Basic admin tasks, minimal meetings, primary focus on payroll processing',
    'senior_consultant': 'Some client interaction, basic training responsibilities, payroll expertise',
    'manager': 'Team management, client meetings, process oversight, staff development',
    'senior_manager': 'Strategic planning, multiple team oversight, extensive client interaction',
    'partner': 'Business development, strategic leadership, extensive admin responsibilities',
    'senior_partner': 'Executive leadership, business strategy, maximum administrative load'
  };
