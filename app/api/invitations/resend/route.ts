...
import { 
  ...
  type GetInvitationByIdQuery,
  type ResendUserInvitationMutation,
  type ValidateInvitationRolePermissionsQuery
} from "@/domains/auth/graphql/generated/graphql";
import { getPermissionsForRole, Role } from "@/lib/auth/simple-permissions";
...
export const POST = withAuth(async (request, { session }) => {
  try {
    ...
    const invitationData = await executeTypedQuery<GetInvitationByIdQuery>(
      GetInvitationByIdDocument,
      { invitationId }
    );
    ...
    const canResend = 
      invitation.invitedBy === session.databaseId || // Original inviter
      hasRoleLevel(session.role, "org_admin"); // Org admin or developer can resend any
    ...
    const updatedInvitation = await executeTypedMutation<ResendUserInvitationMutation>(
      ResendUserInvitationDocument,
      {
        invitationId,
