...
export const GET = withAuth(async req => {
  try {
    ...
    const leaveData = await executeTypedQuery<GetLeaveQuery>(GetLeaveDocument, {
      limit,
      offset,
      where: whereClause,
    });
      ...
      data: {
        ...
        pagination: {
          ...
          totalPages: Math.ceil(
            (leaveData.leaveAggregate?.aggregate?.count || 0) / limit
          ),
        },
      },
    });
...
export const POST = withAuth(async (req, session) => {
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "Missing required fields: userId, startDate, endDate, leaveType",
        },
        { status: 400 }
      );
    }
    ...
    if (managerCreateRequest && managerId) {
      ...
      const canManageTeamLeave = ["manager", "org_admin", "developer"].includes(
        userRole
      );
        ...
        return NextResponse.json(
          {
            success: false,
            error:
              "You can only create leave requests for your direct or indirect reports",
          },
          { status: 403 }
        );
      }
      ...
      if (autoApprove) {
        ...
        const data =
          await executeTypedMutation<CreatePendingLeaveForEmployeeMutation>(
            CreatePendingLeaveForEmployeeDocument,
            {
              userId,
              startDate,
