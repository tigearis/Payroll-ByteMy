...
CREATE VIEW audit.user_access_summary AS
 SELECT u.id AS user_id,
    ...
    count(DISTINCT al.id) FILTER (WHERE (al.action = 'SELECT'::text)) AS read_actions,
    count(DISTINCT al.id) FILTER (WHERE (al.action = ANY (ARRAY['INSERT'::text, 'UPDATE'::text, 'DELETE'::text]))) AS write_actions,
    count(DISTINCT ae.id) FILTER (WHERE ((ae.event_type = 'login'::text) AND (ae.success = true))) AS successful_logins,
    count(DISTINCT ae.id) FILTER (WHERE ((ae.event_type = 'login'::text) AND (ae.success = false))) AS failed_logins,
    max(al.event_time) AS last_activity,
    max(ae.event_time) FILTER (WHERE ((ae.event_type = 'login'::text) AND (ae.success = true))) AS last_login
   FROM ((public.users u
